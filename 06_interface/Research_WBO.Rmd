...
## verdelen. Voor scenario S100 zijn extra woningtypes conform buurt, maar voor  #
...
end_scen <- max(as.numeric(df_scen01_base$jaar[df_scen01_base$scenario == "S100"]))
end_all <- 2040
...
## --- setup variables of scenario
start_scen <- max(as.numeric(df_scen01_past$jaar)) + 1
end_scen <- max(as.numeric(df_scen01_base$jaar[df_scen01_base$scenario == "S130"]))
end_all <- 2040
jrn_pre <- as.numeric(unique(df_scen01_past$jaar))
jrn_scen <- c(start_scen:end_scen)
dur_scen <- length(jrn_scen)
...
### Plot A223 Elektriciteitsverbruiken 2004-2024
...
Het CBS geeft aan dat het gemiddeld autobezit per huishouden in de Randstad **0.6** is.  Tevens heeft PWC voor het Rijk een onderzoek gedaan naar het verwachte aantal elektrische auto's in 2050. Deze gegevens zijn de basis geweest om het aantal elektriscche auto's te prognotiseren.
...
knitr::include_graphics(paste0(dir_cln_pic, "/El_schaarste_Gem.png"))
```
...
el_won_S100 <- prettyNum(df_A6_nr$tot_el_won[df_A6_nr$scenario == "S100"], big.mark = ".", decimal.mark = ",")
el_won_S130 <- prettyNum(df_A6_nr$tot_el_won[df_A6_nr$scenario == "S130"], big.mark = ".", decimal.mark = ",")
el_auto_S100 <- prettyNum(round(df_A6_nr$tot_el_auto[df_A6_nr$scenario == "S100"], 
                          digits = 0), big.mark = ".", decimal.mark = ",")
el_auto_S130 <- prettyNum(round(df_A6_nr$tot_el_auto[df_A6_nr$scenario == "S130"], 
                          digits = 0), big.mark = ".", decimal.mark = ",")

```


### Ruimt.Ontw.Strategie 100% | wonen: `r el_won_S100` | mob: `r el_auto_S100`
...
### Ruimt.Ontw.Strategie 130% | wonen: `r el_won_S130` | mob: `r el_auto_S130`
```{r valuebox_A61b}
## Display value -------------------------------------------------------------
...
### Map A622: **Ruimtelijke Ontw.Strategie (130%)**
...
### Table A631 **Ruimtelijke Ontw.Strategie (100%)**
```{r table_A631}
## prepare table for display
...
### Table A632 **Ruimtelijke Ontw.Strategie (130%)**
```{r table_A632}
## prepare table for display
...
### Map A722: **Ruimtelijke ontwikkelstrategie (130%) **
...
### Table A721 Ruimtelijke ontw.strategie (100%)
```{r table_A721}
sc_code <- "S100"
...
### Table A722 Ruimtelijke ontw.strategie (130%)
```{r table_A722}
sc_code <- "S130"
